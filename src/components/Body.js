...
const Body = () => {
  ...
  const fetchData = async () => {
    ...
    setfilterofResturants(json.data.cards[1].card.card.gridElements.infoWithStyle.restaurants);
    // console.log(json.data.cards[1].card.card.gridElements.infoWithStyle.restaurants);
    ...
    <div className="body">
      <div className="flex">
        <div className="m-4 p-4">
          <input className="border-4 ml-12 " value={searchVal} onChange={(e) => { SetsearchVal(e.target.value) }}></input>
          <button className="px-4 py-2 bg-gray-100 m-4 rounded-lg hover:bg-gray-200 "
            onClick={() => {
              const newList = ListofResturants.filter((res) => res.info.name.toLowerCase().includes(searchVal.toLowerCase()));
        ...
        <button
          className="px-4 py-2 m-4 bg-gray-100 rounded-lg hover:bg-gray-200" onClick={() => {
      ...
      <div className="flex flex-wrap justify-center">
        {filterofResturants.map((restaurant) => {
          ...
          return <Link key={restaurant.info.id} to={"/resturants/" + restaurant.info.id}>   <RestaurantCard resData={restaurant}/>   </Link>;
